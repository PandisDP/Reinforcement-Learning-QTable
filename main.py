...
def qlearning_training(training_iter,size, item_pickup, item_dropoff, start_position, zones_block):
    '''
    Function to train the QLearning model. The first steps is to determine the best hyperparameters and then train the model
    Parameters:
    training_iter: number of iterations to train the model
    ...
    zones_block: list of tuples with the positions of the blocks
    '''
    ...
    hyperparams=qlearning.hyperparameters_training(training_iter,epsilon_valores,alpha_valores,
                                                gamma_valores,min_epsilon,decay_epsilon)
    print('Training Process')
    qlearning.training(training_iter,hyperparams['epsilon'],hyperparams['alpha'],hyperparams['gamma'],
                    min_epsilon,decay_epsilon,True)
...
def qlearning_predict(size, item_pickup, item_dropoff, start_position, 
    ...
    '''
    Function to predict the QLearning model loaded from the file q_table.joblib
    Parameters:
    size: size of the field
    item_pickup: position of the item to pickup
    item_dropoff: position of the item to dropoff
    start_position: position of the agent
    zones_block: list of tuples with the positions of the blocks
    print_episode: boolean to print the episode
    re_training_bool: boolean to retrain the model
...
def Analysis_Prediction(iterations,size, item_pickup, item_dropoff, start_position, 
                        zones_block ,print_episode=False,re_training_bool=False,re_training_epi=1000):
    '''
    Function to analyze the prediction of the QLearning model
    Parameters: 
    iterations: number of iterations to predict the model
    size: size of the field
    item_pickup: position of the item to pickup
    item_dropoff: position of the item to dropoff
    start_position: position of the agent
    ...
    for i in range(iterations):
        steps,reward,status_all= qlearning_predict(size, item_pickup, item_dropoff, 
                start_position, zones_block ,print_episode,re_training_bool,re_training_epi)
...
def qtable_analysis(name_file='q_table.joblib',iterations=50):
    '''
    Function to analyze the QLearning model
    Parameters:
    name_file: name of the file to load the model
    iterations: number of iterations to analyze the model
    '''
    size=10
    item_pickup=(0,0)
    item_dropoff=(7,9)
    ...
    qlearning_predict.analysys_qtable_fig(name_file,iterations)
    qlearning_predict.analysys_qtable(name_file)
...
if __name__ == "__main__":
    ...
    #qtable_analysis('q_table.joblib',100)
    #Analysis_Prediction(10000,size, item_pickup, item_dropoff, start_position, zones_block,False,False,10000)
